	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'SendPlainText':
				return _Utils_Tuple2(
					model,
					A2($author$project$Port$Middleware$sendString, $author$project$Port$Middleware$AllowTextOnly, model.plainText));
			case 'SendRichText':
				return _Utils_Tuple2(
					model,
					A2($author$project$Port$Middleware$sendString, $author$project$Port$Middleware$AllowSafeHtml, model.richText));
			case 'SendUrl':
				return _Utils_Tuple2(
					model,
					...
					var elmResults = A2(
						...
						function (p) {
							return {
								...
								dpSafeHtml: '',
								dpTextOnly: '',
								hpSafeHtml: A2($author$project$Port$Middleware$sanitize, $author$project$Port$Middleware$AllowSafeHtml, p.input),
								hpTextOnly: A2($author$project$Port$Middleware$sanitize, $author$project$Port$Middleware$AllowTextOnly, p.input),
								id: p.id,
								input: p.input,
								regexSafeHtml: A2($author$project$Port$Middleware$sanitizeRegex, $author$project$Port$Middleware$AllowSafeHtml, p.input),
								regexTextOnly: A2($author$project$Port$Middleware$sanitizeRegex, $author$project$Port$Middleware$AllowTextOnly, p.input)
							};
						},
						payloads);
					return _Utils_Tuple2(
...
var $author$project$Tests$allowSafeHtmlTests = _List_fromArray(
	[
		{
...
var $author$project$Tests$allowTextOnlyTests = _List_fromArray(
	[
		...
		policy: $author$project$Port$Middleware$AllowTextOnly,
		shouldBlock: _List_fromArray(
			['<script']),
		shouldPass: _List_Nil
	},
...
var $author$project$Tests$allTests = _Utils_ap(
	...
	_Utils_ap($author$project$Tests$allowSafeHtmlTests, $author$project$Tests$allowUrlTests));
...
var $author$project$Tests$runTest = function (tc) {
	...
	var blockViolations = A2(
		...
		A2(
			...
			tc.shouldBlock));
	var failures = _Utils_ap(blockViolations, passViolations);
	return {
		failures: failures,
		output: output,
...
var $author$project$Tests$runAllTests = A2($elm$core$List$map, $author$project$Tests$runTest, $author$project$Tests$allTests);
...
var $author$project$Main$tabButton = F3(
	function (tab, label, activeTab) {
		...
		var activeStyles = _List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'border-bottom', '2px solid #2563eb'),
...
var $author$project$Main$viewComparisonRow = function (row) {
	var regexMatch = _Utils_eq(row.regexOutput, row.dpOutput);
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							$elm$core$String$isEmpty(s) ? '(empty)' : A2($author$project$Main$truncate, 40, s))
						]))
				]));
	};
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								A2($author$project$Main$truncate, 40, row.input))
							]))
					])),
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Confusion matrix — ' + label)
						])),
					A2(
					$elm$html$Html$table,
					_List_fromArray(
...
var $author$project$Main$viewMetricsPanel = F2(
	function (label, m) {
		...
		return A2(
			...
			_List_fromArray(
				[
					A2($author$project$Main$viewConfusionMatrix, label, m),
					A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
						...
						[
							...
							_List_fromArray(
								[
									...
									'TP / (TP+FP)',
									'Of ' + (label + '\'s blocks, what fraction produced the exact same output as DOMPurify.')),
									A4(
									$author$project$Main$viewMetricCard,
									'Recall',
									$author$project$Main$formatPct(rec),
									'TP / (TP+FN)',
									'Of cases where outputs differed, what fraction did ' + (label + ' block with matching output.')),
									A4(
									$author$project$Main$viewMetricCard,
									'F1 Score',
...
var $author$project$Main$viewComparisonSection = F4(
	function (title, color, note, rows) {
		...
		var regexRows = A2(
			$elm$core$List$map,
			function (r) {
				return {dpOutput: r.dpOutput, elmOutput: r.regexOutput, input: r.input};
			},
			rows);
		var regexMatrix = $author$project$Main$computeMatrix(regexRows);
		...
		var hpRows = A2(
			$elm$core$List$map,
			function (r) {
				return {dpOutput: r.dpOutput, elmOutput: r.hpOutput, input: r.input};
			},
			rows);
		var hpMatrix = $author$project$Main$computeMatrix(hpRows);
								...
								[
									...
									_List_fromArray(
										[
											...
											_List_fromArray(
												[
													$elm$html$Html$text('Regex vs DOMPurify')
												])),
											A2(
											$author$project$Main$badge,
											$elm$core$String$fromInt(regexMatchCount) + ' match',
											'#16a34a'),
											A2(
											$author$project$Main$badge,
											$elm$core$String$fromInt(regexDivergeCount) + ' diverge',
											(!regexDivergeCount) ? '#9ca3af' : '#d97706')
										])),
									A2($author$project$Main$viewMetricsPanel, 'Regex', regexMatrix)
								])),
							A2(
							$elm$html$Html$div,
							_List_fromArray(
								...
								[
									...
									_List_fromArray(
										[
											...
											_List_fromArray(
												[
													$elm$html$Html$text('HTML Parser vs DOMPurify')
												])),
											A2(
											$author$project$Main$badge,
											$elm$core$String$fromInt(hpMatchCount) + ' match',
											'#16a34a'),
											A2(
											$author$project$Main$badge,
											$elm$core$String$fromInt(hpDivergeCount) + ' diverge',
											(!hpDivergeCount) ? '#9ca3af' : '#d97706')
										])),
									A2($author$project$Main$viewMetricsPanel, 'HTML Parser', hpMatrix)
								]))
						])),
					A2(
					$elm$html$Html$div,
										...
										[
											...
											_List_fromArray(
												[
													A2(
													$elm$html$Html$th,
													A2($author$project$Main$thStyle, '110px', 'left'),
													_List_fromArray(
														[
													...
													A2(
													$elm$html$Html$th,
													A2($author$project$Main$thStyle, '160px', 'left'),
													_List_fromArray(
														[
															$elm$html$Html$text('Regex output')
														])),
													A2(
													$elm$html$Html$th,
													A2($author$project$Main$thStyle, '160px', 'left'),
													_List_fromArray(
														[
															$elm$html$Html$text('HTML Parser output')
														])),
													A2(
													$elm$html$Html$th,
													A2($author$project$Main$thStyle, '160px', 'left'),
		...
		_List_fromArray(
			[
				...
				A2(
					$elm$core$List$map,
					function (r) {
						return {category: r.category, dpOutput: r.dpTextOnly, hpOutput: r.hpTextOnly, input: r.input, regexOutput: r.regexTextOnly};
					},
					model.comparisonResults)),
				A4(
				$author$project$Main$viewComparisonSection,
				...
				A2(
					$elm$core$List$map,
					function (r) {
						return {category: r.category, dpOutput: r.dpSafeHtml, hpOutput: r.hpSafeHtml, input: r.input, regexOutput: r.regexSafeHtml};
					},
					model.comparisonResults))
			])));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A3($author$project$Main$demoInput, 'Enter text (try XSS payloads)', model.plainText, $author$project$Main$UpdatePlainText),
						A3($author$project$Main$demoButton, 'Send with AllowTextOnly', $author$project$Main$SendPlainText, '#dc2626')
					])),
				A5(
				$author$project$Main$policyCard,
				'2',
				...
				_List_fromArray(
					[
						A3($author$project$Main$demoInput, 'Enter HTML content', model.richText, $author$project$Main$UpdateRichText),
						A3($author$project$Main$demoButton, 'Send with AllowSafeHtml', $author$project$Main$SendRichText, '#d97706')
					])),
				A5(
				$author$project$Main$policyCard,
				'3',
				...
				_List_fromArray(
					[
						A3($author$project$Main$demoInput, 'Enter URL', model.url, $author$project$Main$UpdateUrl),
						A3($author$project$Main$demoButton, 'Send with AllowUrl', $author$project$Main$SendUrl, '#2563eb')
					])),
				A5(
				$author$project$Main$policyCard,
				'4',
				...
				[
					...
					_List_fromArray(
						[
							...
							A2($elm$html$Html$Attributes$style, 'max-width', '380px')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(
							A2($author$project$Main$truncate, 80, content))
						]))
				]));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'margin-bottom', '6px'),
				A2($elm$html$Html$Attributes$style, 'background', passColor)
			]),
		_List_fromArray(
			[
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(result.testCase.description)
									])),
								A2(
								$elm$html$Html$div,
								_List_fromArray(
								...
								_List_fromArray(
									[
										A2($author$project$Main$codeSnippet, 'IN', result.testCase.input),
										A2($author$project$Main$codeSnippet, 'OUT', result.output)
									])),
								(!result.passed) ? A2(
								$elm$html$Html$div,
								_List_fromArray(
								...
								A2(
									...
									result.failures)) : $elm$html$Html$text('')
							]))
					]))
			]));
};
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromInt(passed) + ('/' + $elm$core$String$fromInt(total)))
								]))
						])),
					A2(
					$elm$html$Html$div,
					_List_Nil,
					A2($elm$core$List$map, $author$project$Main$viewTestRow, results))
				]));
	});
var $author$project$Main$viewTestGroups = function (results) {
	var categories = _List_fromArray(
		['AllowTextOnly', 'AllowSafeHtml', 'AllowUrl']);
	return A2(
		$elm$core$List$map,
		function (cat) {
			return A2(
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$style,
						'border',
						'1px solid ' + ((!failed) ? '#bbf7d0' : '#fecaca')),
						A2($elm$html$Html$Attributes$style, 'border-radius', '8px'),
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								(!failed) ? 'All tests passing' : 'Some tests failing')
							])),
						A2(
						$author$project$Main$badge,
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								$elm$core$String$fromInt(total) + ' total')
							]))
					])),
				A2(
				$elm$html$Html$div,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'max-width', '80dvw'),
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Elm XSS Protection — Port Middleware Demo')
					])),
				A2(
				$elm$html$Html$p,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Proof-of-concept library demonstrating compile-time policy enforcement at the Elm → JavaScript port boundary.')
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
